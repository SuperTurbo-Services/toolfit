...
if [ ! -f "$MUSIC" ]; then
  ...
  if [ -z "$AVAIL" ]; then
    echo "" >&2
    echo "  No BGM tracks are bundled with this skill. The six mood presets" >&2
    echo "  (tech, ad, educational, educational-alt, tutorial, tutorial-alt) were" >&2
    echo "  left out deliberately: they are 26.6 MB of the 33 MB upstream project," >&2
    echo "  which is 80.6% of its weight, and they are not needed to make the" >&2
    echo "  animation. Only the mixing recipe is." >&2
    echo "" >&2
    echo "  Two ways forward:" >&2
    echo "    1. Bring your own track:  bash add-music.sh in.mp4 --music=/path/to/track.mp3" >&2
    echo "    2. Restore the presets:   download bgm-<mood>.mp3 from" >&2
    echo "       https://github.com/alchaincyf/huashu-design/tree/main/assets" >&2
    echo "       and drop them into $ASSETS_DIR" >&2
  else
    echo "  Available moods: $AVAIL" >&2
  fi
  exit 1
fi
